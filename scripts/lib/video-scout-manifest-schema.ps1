...
$script:VideoScoutSchemaVersions = @(1, 2, 3, 4)

# V4Q: the canonical diagnostic-artifact kind and the hard cap on the array. At most ONE diagnostic
# can exist per run -- a run submits at most one logical request and the quality gate is terminal,
# so a second entry would mean the fail-closed lifecycle was bypassed.
$script:VideoScoutDiagnosticKinds = @('quality-rejected-response')
$script:VideoScoutMaxDiagnosticArtifacts = 1
$script:VideoScoutDiagnosticKeys = @('kind', 'fileName', 'sha256', 'bytes')
# Declared here as the MANIFEST contract's own copy, deliberately independent of the enforcement
# copies in scripts/lib/get-video-scout-diagnostic-artifact.ps1 (the verifier) and
# scripts/gemini-video-sdk.js (the producer). Tests pin all three to equal values, so a change in
# one layer fails loudly instead of letting the manifest accept something the gate never produced.
$script:VideoScoutDiagnosticFileNameConst = 'rejected-response.txt'
$script:VideoScoutMaxDiagnosticBytesConst = 4 * 1024 * 1024
$script:VideoScoutQualityReasonCodes = @(
    'finish-max-tokens'
    'finish-not-stop'
    'missing-section'
    'duplicate-section'
...
function Get-VideoScoutSchemaVersions { $script:VideoScoutSchemaVersions }
function Get-VideoScoutQualityReasonCodes { $script:VideoScoutQualityReasonCodes }
function Get-VideoScoutSchemaDiagnosticFileName { $script:VideoScoutDiagnosticFileNameConst }
function Get-VideoScoutSchemaMaxDiagnosticBytes { $script:VideoScoutMaxDiagnosticBytesConst }
...
<#
.SYNOPSIS
  V4Q (v4): validate the diagnosticArtifacts array and its coupling to the run's terminal state.
.DESCRIPTION
  A diagnostic records that a provider response was REJECTED locally and preserved as evidence. The
  structural guarantees enforced here are what stop that evidence from ever being mistaken for
  output:
    * at most ONE entry (a run submits at most one logical request; the gate is terminal);
    * an exact entry shape -- kind/fileName/bytes/sha256 and nothing else;
    * a NONEMPTY array REQUIRES outcome='error', reportFile=null, and an allowlisted quality code in
      the reason. A rejected response therefore cannot coexist with a completed run or a report.
  Library availability keys off outcome + reportFile, so these couplings are exactly what keep a
  diagnostic run permanently unavailable/incomplete rather than readable.
#>
function Assert-VideoScoutDiagnosticArtifactsValid {
    param([Parameter(Mandatory)]$Manifest)
    $diags = $null
    if ($Manifest -is [System.Collections.IDictionary]) {
        if ($Manifest.Contains('diagnosticArtifacts')) { $diags = $Manifest['diagnosticArtifacts'] }
    }
    else {
        $prop = $Manifest.PSObject.Properties['diagnosticArtifacts']
        if ($prop) { $diags = $prop.Value }
    }
    if ($null -eq $diags) { throw 'Manifest validation failed (v4): diagnosticArtifacts must be an array (got null).' }
    if (($diags -is [string]) -or -not (($diags -is [System.Array]) -or ($diags -is [System.Collections.IList]))) {
        throw 'Manifest validation failed (v4): diagnosticArtifacts must be an array, never an arbitrary object.'
    }
    $items = @($diags)
    if ($items.Count -gt $script:VideoScoutMaxDiagnosticArtifacts) {
        throw "Manifest validation failed (v4): diagnosticArtifacts may hold at most $($script:VideoScoutMaxDiagnosticArtifacts) entry (got $($items.Count))."
    }
    ...
    if ($items.Count -eq 0) {
        return   # the normal case: every non-rejected run, including every completed one
    }

    # ---- a preserved diagnostic exists: the run is terminally an error with no report --------------
    if ($outcome -ne 'error') {
        throw "Manifest validation failed (v4): a manifest with a diagnosticArtifacts entry must record outcome='error' (got $(if ($null -eq $outcome) { 'null' } else { "'$outcome'" })); a rejected response is never a completed run."
    }
    if ($null -ne $reportFile) {
        throw 'Manifest validation failed (v4): a manifest with a diagnosticArtifacts entry must record reportFile=null; a rejected response is never a report.'
    }
    if ([string]::IsNullOrWhiteSpace([string]$reason) -or ([string]$reason -notmatch '^\[quality:([a-z0-9\-]+)\]')) {
        throw 'Manifest validation failed (v4): a manifest with a diagnosticArtifacts entry must record a reason beginning with an allowlisted [quality:<code>] marker.'
    }
    $code = $Matches[1]
    if ($script:VideoScoutQualityReasonCodes -cnotcontains $code) {
        throw "Manifest validation failed (v4): '$code' is not an allowlisted V4Q quality-gate failure code."
    }

    $entry = $items[0]
    if ($null -eq $entry) { throw 'Manifest validation failed (v4): diagnosticArtifacts entry 1 is null.' }
    if (($entry -is [string]) -or ($entry -is [ValueType]) -or ($entry -is [System.Array]) -or ($entry -is [System.Collections.IList])) {
        throw 'Manifest validation failed (v4): diagnosticArtifacts entry 1 must be an object.'
    }
    $keys = Get-ManifestKeyList -M $entry
    $missing = @($script:VideoScoutDiagnosticKeys | Where-Object { $keys -notcontains $_ })
    $extra   = @($keys | Where-Object { $script:VideoScoutDiagnosticKeys -notcontains $_ })
    if ($missing.Count) { throw "Manifest validation failed (v4): diagnosticArtifacts entry 1 is missing key(s): $($missing -join ', ')." }
    if ($extra.Count)   { throw "Manifest validation failed (v4): diagnosticArtifacts entry 1 has unknown key(s): $($extra -join ', ')." }

    $kind     = Get-ManifestValue -M $entry -Key 'kind'
    $fileName = Get-ManifestValue -M $entry -Key 'fileName'
    $bytes    = Get-ManifestValue -M $entry -Key 'bytes'
    $sha256   = Get-ManifestValue -M $entry -Key 'sha256'

    if ($script:VideoScoutDiagnosticKinds -cnotcontains $kind) {
        throw "Manifest validation failed (v4): diagnosticArtifacts entry 1 kind must be one of $($script:VideoScoutDiagnosticKinds -join '/')."
    }
    # The leaf is a repository constant, never a provider- or caller-supplied name.
    if ($fileName -cne $script:VideoScoutDiagnosticFileNameConst) {
        throw "Manifest validation failed (v4): diagnosticArtifacts entry 1 fileName must be exactly '$($script:VideoScoutDiagnosticFileNameConst)'."
    }
    if ($null -eq $bytes -or -not (($bytes -is [int]) -or ($bytes -is [long]))) {
        throw 'Manifest validation failed (v4): diagnosticArtifacts entry 1 bytes must be a whole number.'
    }
    if ([long]$bytes -lt 0 -or [long]$bytes -gt $script:VideoScoutMaxDiagnosticBytesConst) {
        throw "Manifest validation failed (v4): diagnosticArtifacts entry 1 bytes must be from 0 to $($script:VideoScoutMaxDiagnosticBytesConst)."
    }
    if (($sha256 -isnot [string]) -or ($sha256 -cnotmatch '^[0-9a-f]{64}$')) {
        throw 'Manifest validation failed (v4): diagnosticArtifacts entry 1 sha256 must be a lowercase 64-character hex digest.'
    }
}
...
function New-VideoScoutLiveManifest {
    ...
    $hasSlices = ($null -ne $SliceRanges -and $sliceList.Count -gt 0)
    $sliceEntries = {
    ...
    # V4Q: EVERY new live SDK run is schema version 4 -- whole-video, scalar, and multipart alike.
    # v4 always carries all three collection fields so the shape is stable across slice scopes:
    # mediaArtifacts (required EMPTY -- the SDK route owns no local media, which is what keeps a v4
    # manifest permanently outside every media ownership/deletion path), requestedSliceRanges (null
    # unless multipart -- the scalar offsets stay authoritative for one slice), and diagnosticArtifacts
    # (empty until a response is rejected locally). Existing v1/v2/v3 manifests are NOT migrated;
    # CLI runs keep their v2 behavior; backfills stay v1.
    if ($Route -eq 'sdk') {
        $m.schemaVersion        = 4
        $m.requestedSliceRanges = $null
        $m.diagnosticArtifacts  = @()
        if ($hasSlices) {
            $m.startOffsetSeconds   = $null
            $m.endOffsetSeconds     = $null
            $m.requestedSliceRanges = & $sliceEntries
        }
    }
    elseif ($hasSlices) {
        # Not reachable in production (slices are refused on every non-SDK route before a run dir
        # exists). Kept exact so a caller bug produces the SAME loud v3 route validation failure it
        # always did, rather than a silently slice-less manifest.
        $m.schemaVersion        = 3
        $m.startOffsetSeconds   = $null
        $m.endOffsetSeconds     = $null
        $m.requestedSliceRanges = & $sliceEntries
...
function Assert-VideoScoutManifestValid {
    ...
    if ($schemaVersion -eq 4) {
        # V4Q: version 4 = ANY new live SDK run. It always carries all three collection fields so the
        # shape is stable across whole-video / scalar / multipart scopes, and is never a backfill.
        if ($isBackfill) { throw 'Manifest validation failed: a schemaVersion 4 manifest must not be a backfill (backfills remain version 1 — scope, ownership, and diagnostics are never fabricated for history).' }
        $expected = @($baseKeys + 'mediaArtifacts' + 'requestedSliceRanges' + 'diagnosticArtifacts')
    }
    elseif ($schemaVersion -eq 3) {
        # V4: version 3 = a multi-slice SDK run. It carries BOTH v2's mediaArtifacts (which must be
        # empty) and requestedSliceRanges, and is never a backfill. Existing v3 history stays valid
        # unchanged and is never migrated to v4.
    ...
    # V4Q (v4): the same media-inventory rules apply AND the inventory must be EMPTY -- that is the
    # structural guarantee a v4 manifest is never media-ownership or deletion authority. The slice
    # set is OPTIONAL here (null for whole-video and scalar runs, a validated 2-8 array for
    # multipart), and the diagnostic array is validated together with its outcome/reportFile/reason
    # coupling.
    if ($schemaVersion -eq 4) {
        Assert-VideoScoutMediaArtifactsValid -Manifest $Manifest
        $v4Media = @(Get-ManifestValue -M $Manifest -Key 'mediaArtifacts')
        if ($v4Media.Count -ne 0) {
            throw 'Manifest validation failed (v4): mediaArtifacts must be EMPTY on a schemaVersion 4 manifest (the SDK route owns no local media; a v4 manifest is never media-ownership or deletion authority).'
        }
        if ($null -ne (Get-ManifestValue -M $Manifest -Key 'requestedSliceRanges')) {
            Assert-VideoScoutSliceRangesValid -Manifest $Manifest
        }
        Assert-VideoScoutDiagnosticArtifactsValid -Manifest $Manifest
    }
    ...
    if (-not $isBackfill) {
        ...
        # V4Q (v4): a version 4 manifest is SDK-only by construction. When it records a slice SET,
        # that set is the authoritative scope and the scalar offsets must be null -- two competing
        # scope records could otherwise disagree about what was requested. A v4 run WITHOUT a slice
        # set is whole-video or scalar, and there the scalar offsets remain authoritative.
        if ($schemaVersion -eq 4) {
            if ($route -ne 'sdk') {
                throw "Manifest validation failed (v4): a schemaVersion 4 manifest must record route='sdk' (version 4 is the live SDK Video Scout schema; CLI runs keep version 2)."
            }
            if ($null -ne (Get-ManifestValue -M $Manifest -Key 'requestedSliceRanges')) {
                if ($null -ne $startOff -or $null -ne $endOff) {
                    throw 'Manifest validation failed (v4): startOffsetSeconds and endOffsetSeconds must both be null on a multi-slice manifest (requestedSliceRanges is the authoritative scope).'
                }
            }
        }
