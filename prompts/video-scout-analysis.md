...
Duration: state it only if you can actually see the full source (for example a visible player duration you quote verbatim). If you were given only a bounded portion of the video, you CANNOT determine the total length — say so rather than estimating, and report only the authorized duration you were given.
